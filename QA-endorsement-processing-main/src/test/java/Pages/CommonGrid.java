...
//            return driver.findElement(By.xpath("//table[@id='"+gridMap.get(tabName)+"']/descendant::th[normalize-space(@aria-label)='"+header+"']")).isDisplayed();
    ...
    private void initializeMaps() {
        ...
        gridMap.put("importantinstructionsreset", "impIns");

        // Time Records
        gridMap.put("timerecordspage", "dtTimeRecord");
        gridMap.put("timerecordadmin", "dtTimeRecordAdmin");
        gridMap.put("timerecordopen", "dtTimeRecord");
    }
...
//                "//th[normalize-space(@aria-label)='"+header+"']/descendant::input[contains(@placeholder, '"+fromTo+"')]"));
    ...
    public boolean gridHeaderDateSelect(String header, String fromTo, String day, String month, String year) {
        ...
        WebElement pickerMonthYear = driver.findElement(By.xpath("//div[contains(@class, 'picker-days')]/descendant::th[contains(@class, 'switch')]"));
        WebElement pickerYear = driver.findElement(By.xpath("//div[contains(@class, 'picker-months')]/descendant::th[contains(@class, 'switch')]"));
        WebElement pickerNext = driver.findElement(By.xpath("//div[contains(@class, 'picker-months')]/descendant::th[@class='next']"));
        WebElement pickerPrev = driver.findElement(By.xpath("//div[contains(@class, 'picker-months')]/descendant::th[@class='prev']"));
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(@class, 'picker-days')]" +
...
//            wait.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(xPath)));
    ...
    public boolean clickResetButton(String tab) {
        try {
            WebElement resetBtn;
            System.out.println("going to try to find the reset button at \"//div[@id='tab" + gridMap.get(tab.toLowerCase(Locale.ROOT)+"reset") +"']//div[contains(@class, 'btn btn-sm red btn-outline search-clear')]\"");
            resetBtn = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='tab" + gridMap.get(tab.toLowerCase(Locale.ROOT).replaceAll("\\s", "")+"reset") + "']//div[contains(@class, 'btn btn-sm red btn-outline search-clear')]")));
            resetBtn.click();
            System.out.println("Clicking Reset Button");
            return true;
        } catch (Exception e) {
            System.out.println("Cannot Find Reset Button");
            return false;
        }

    }

    public String gridCount(String tabName) {
        //tabs are open = 'processing', completed = 'completed', discarded = 'discarded', important instructions = 'impIns'
        final Set<String> strings = new HashSet<String>() {{
            ...
            add("time records");
            add("wo details time records");
        }};

        System.out.println("Counting records in the " + tabName + " tab");
        if (tabName == null){
            tabName = "open";
        } else if (!strings.contains(tabName.toLowerCase())){
            System.out.println("value of tabName must be either Open, Completed, Discarded or Important Info");
        }
        String countString = null;
        try {
            countString = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='" +  gridMap.get(tabName.toLowerCase(Locale.ROOT).replaceAll("\\s", "")) + "_info' and @role='status']")))
                    .getText()
                    .replaceAll("[^0-9]+", " ")
                    .replaceAll("\\s+","");
            if(countString.equals("")){
                ...
                System.out.println("the record count is: " + countString);
            }
            return countString;
        } catch (Exception e) {
            System.out.println("Cannot find record count");
            return countString;
        }
    }
    public String verifyBackGroundColor(String tabName){
        String colorElement = null;
        int attempts = 0;
        String currentTab = gridMap.get(tabName.toLowerCase().replaceAll("\\s+", ""));

        //while loop avoids a StaleElementReferenceException if the filter is not fast enough
        while(attempts < 2) {
            try {
                Thread.sleep(500);
                WebElement gridFirstRow = driver.findElement(By.xpath("//*[@id='" + currentTab + "']/tbody/tr[1]"));
                colorElement = gridFirstRow.getCssValue("background-color");
                break;
            } catch (java.util.NoSuchElementException e) {
                System.out.println("Was unable to find the first row in the " + currentTab + " grid.");
            } catch (InterruptedException e) {
                System.out.println("sleep broke");
            }
            attempts++;
        }
        return colorElement;
     ...
     */
    public int waitForFilter(String originalTopRecordID, String header) {
        int result = waitForFilter(originalTopRecordID, header, 3000);
        return result;
    }
     ...
     * */
    public int waitForFilter(String originalTopRecordID, String header, long waitLength) {
        int result = -1;
