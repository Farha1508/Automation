...
public abstract class NodeApp implements ICommonElements {
    ...
    public boolean commonCheckBox(String checkBox) {
        ...
        List<WebElement> checkBoxes3 = driver.findElements(By.xpath(
                "//label[normalize-space(text())='" + checkBox + "']/input[@type='checkbox']"));
    ...
    public WebElement commonField(String field) {
        ...
        //List to handle fields in Orders/Reminders page
        List<WebElement> fields4 = driver.findElements(By.xpath("//input[@placeholder= '"+field+"']"));
    ...
    public String gridHeaderEnter(String headerName, String input) {
        ...
        //this for loop deliberately slows down the text input to ensure the entire query is entered as filters will frequently
        // update before the entire search term is entered, which can trigger failed assertions
        try {
            for (int i = 0; i < input.length(); i++) {
                String ch = String.valueOf(input.charAt(i));
                selectedHeader.sendKeys(ch);
                Thread.sleep(100);
            }
        } catch (Exception e)
        {
            System.out.println("Something went wrong when trying to input search");
        }
        selectedHeader.click();
        return null;
    }
