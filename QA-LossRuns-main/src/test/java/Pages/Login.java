...
public class Login extends BaseUtil {
    ...
    private final Properties logins = new Properties();
    ...
    public void BaseUrl(String site) throws IOException {
        String url = "";
        Properties config = new Properties();
        config.load(getClass().getClassLoader().getResourceAsStream("config.properties"));
        String env = config.getProperty("environment");
        if (env.equals("dev")) {
            switch (site.toLowerCase().replaceAll("\\s+", "")) {
                case "patraone":
                    url = "https://dev.patracorp.net/";
                    break;
                case "alphaone":
                    url = "https://alphadev1.patracorp.net/";
                    break;
                case "patraonecloud":
                    url = "http://login.dev.patracloud.net/auth";
                    break;
                default:
                    org.testng.Assert.fail("No development URL found for " + site);

            }
        } else if (env.equals("release")) {
            switch (site.toLowerCase().replaceAll("\\s+", "")) {
                case "patraone":
                    url = "https://release.patracorp.net/";
                    break;
                case "alphaone":
                    url = "https://alpharelease.patracorp.net:8443/CPanelAdmin.a5w";
                    break;
                case "patraonecloud":
                    url = "http://login.release.patracloud.net/auth";
                    break;
                default:
                    org.testng.Assert.fail("No production URL found for " + site);

            }
        } else {
            org.testng.Assert.fail("No URLs found for " + env + " environment");
        }
        driver.get(url);
    ...
    public void EnterCredentials(String user) throws Exception {
        ...
        switch (user.toLowerCase()) {
            case "super user":
                ...
                break;
            case "user role":
                email = login.getProperty("user_email");
                pass = login.getProperty("user_password");
                break;
            case "base qa user":
                email = login.getProperty("qa_user_email");
                pass = login.getProperty("qa_user_password");
                break;
            default:
