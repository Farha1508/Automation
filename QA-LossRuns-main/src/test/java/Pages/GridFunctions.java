...
public class GridFunctions {
    ...
    public GridFunctions(WebDriver driver) {
        this.driver = driver;
    ...
    public boolean gridTab(String tabName) {
        ...
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@id=\""+gridMap.get(tabFix)+"\"]/thead//tr/th")));
    ...
    public boolean gridHeaderFind(String tab, String header) {
        ...
        try {
            return driver.findElement(By.xpath("//table[@id='"+gridMap.get(tabName)+"']/descendant::th[starts-with(@aria-label ,'"+header+"')]")).isDisplayed();
        } catch (Exception e) {
            return false;
        }
    ...
    public String gridHeaderField(String headerName, String input) throws InterruptedException {
        WebElement selectedHeader = null;
        List<WebElement> headers = driver.findElements(By.xpath("" +
    ...
    public boolean gridHeaderSelectorSelect(String headerName, String selection) throws InterruptedException {
    ...
    public boolean gridHeaderDateSelect(String header, Map<String, String> selections) {
        ...
        List<WebElement> pMY = driver.findElements(By.xpath("//div[@class='datepicker-days']/descendant::th[@class='datepicker-switch']"));
        ...
        String curMY = pickerMonthYear.getText().replaceAll("\\s+", "");
        System.out.println(curMY);
        String curMonth = curMY.replaceAll("\\d", "");
        String curYear = curMY.replaceAll("\\D", "");

        if (!selections.get("Month").equalsIgnoreCase(curMonth) || !selections.get("Year").equalsIgnoreCase(curYear)) {
            pickerMonthYear.click();
            List<WebElement> pY = driver.findElements(By.xpath("//div[@class='datepicker-months']/descendant::th[@class='datepicker-switch']"));
            for (WebElement ele : pY) {
                if(ele.isDisplayed()) {
                    pickerYear = ele;
                }
            }
            List<WebElement> pN = driver.findElements(By.xpath("//div[@class='datepicker-months']/descendant::th[@class='next']"));
            for (WebElement ele : pN) {
                if(ele.isDisplayed()) {
                    pickerNext = ele;
                }
            }
            List<WebElement> pP = driver.findElements(By.xpath("//div[@class='datepicker-months']/descendant::th[@class='prev']"));
        ...
        List<WebElement> pD = driver.findElements(By.xpath("//div[@class='datepicker-days']" +
                "/descendant::td[contains(@class, 'day') and not(contains(@class, 'new')) and " +
        ...
        List<WebElement> pH = driver.findElements(By.xpath(
                "//div[@class='datetimepicker-hours']/descendant::legend[normalize-space()='"+selections.get("AM / PM")+"']/following-sibling::span[normalize-space()='hour']")); //insert "+hour+"
    ...
    public WebElement gridEntry(String tableRow, String tableColumn) {
        ...
        if (tableRow.toLowerCase().contains("row")) {
            ...
            List<WebElement> setRow = driver.findElements(By.xpath("" +
                    "//td[.//text()[normalize-space()=\"" + tableRow + "\"]]"));
...
//            wait.until(ExpectedConditions.invisibilityOfElementLocated(By.xpath(xPath)));
    ...
    public void waitForFilter(String originalTopRecordID) {
        ...
        */
        ...
        // originalTopRecordID = gridEntry("row 1", "Record ID").getText();
        long waitLength = 3000; // Wait up to 3 seconds, but maybe this value could be set somewhere else.
        long startTime = System.currentTimeMillis();
        try {
            while (gridEntry("row 1", "Work Order #").getText().equals(originalTopRecordID) && (System.currentTimeMillis() - startTime) < waitLength) {
                try {
        ...
        if(gridEntry("row 1", "Work Order #").getText().equals(originalTopRecordID)){
    ...
    public void enterduration(String duration) throws InterruptedException {
        List<WebElement> durt = driver.findElements(By.xpath("//input[@class='form-control hour-filter']"));
        float min = Float.parseFloat(duration)-1;
        float max = Float.parseFloat(duration)+1;
        durt.get(0).sendKeys(""+min);
        durt.get(1).sendKeys(""+max);
    }

    public void clickTopWorkOrder(String tabName) {
        String currentTab = gridMap.get(tabName.toLowerCase().replaceAll("\\s+", ""));
        try{
            driver.findElement(By.xpath("//*[@id='"+currentTab+"']/tbody/tr[1]/td[2]/a")).click();
        }
        catch (NoSuchElementException e) {
            driver.findElement(By.xpath("//*[@id='"+currentTab+"']/tbody/tr/td[2]/a")).click();
        }
    }
}
